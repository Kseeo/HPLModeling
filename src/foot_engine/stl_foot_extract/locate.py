"""발 후보 위치 자동 탐색 -- 메쉬 형상만으로 "발일 가능성"을 점수화하는
휴리스틱. 확정 판별기가 아니라 사람이 확인할 후보 순위 보조 도구
(picker.py/pipeline.py에서 사용).

세 국소 신호를 합성:
- 밀도(density): 발은 배경보다 카메라를 가까이 대고 찍혀 정점 밀도가 높은
  경향(실측: 진짜 발 위치가 밀도 상위 85 백분위). 단독으론 불완전, 아래
  두 신호와 합성해서만 사용.
...
def find_dense_regions(
    ...
    *,
    ...
    """표면 샘플점 중 국소 밀도 상위 density_top_ratio만 남겨 공간적으로
    군집화한다 -- "정점이 빽빽하게 몰린 구역"을 직접 찾는 방식.

    - 전제: 발이 배경보다 촘촘하게 찍힘. 배경이 밀도 낮은 다리로만 연결돼
      있으면 잘 갈라지지만, 발과 배경이 밀도 차이 없이 진짜로 fuse돼 있으면
      (예: project_223) 파라미터를 조여도 원리적으로 안 갈라짐(실측: eps
      0.8~1.3배 범위에서 최대 구역 크기 거의 불변) -- 그런 케이스는
      picker.py로 좌표 확인 후 crop.py의 crop_around_seed/remove_near_point 사용.
