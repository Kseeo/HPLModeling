...
**마스크 기반 (`masks_dir` 지정 시, 권장)**: 각 3D점이 어느 사진들에서 관측됐는지(SfM이
이미 계산해둔 track)를 이용해, 그 점의 관측 픽셀들이 각 사진의 (`masking.py`) 마스크
안쪽에 찍히는 비율로 발/배경을 분류한다. 특징점 추출 단계에서 마스킹하는 것과
달리 이건 재구성이 다 끝난 뒤 점을 분류만 하는 것이라 등록률에 전혀 영향을
주지 않는다. 평면 가정이 없어 아래 방식의 실패 유형(발을 가로지르는 오분류)이
구조적으로 없다.

**마스크 없을 때(폴백)**: SfM이 계산해둔 카메라 위치·시선 정보로 기하학적
추정을 한다.

    1. 평면 제거(RANSAC) — 배경은 대부분 바닥/책상/벽 같은 넓은 평면이고,
       발은 절대 평면이 아니다. 지배적인 평면을 찾아 그 근처 점을 제거한다.
    2. 공통 초점 거리 필터 — 카메라들의 시선(viewing ray)이 수렴하는 지점을
       최소제곱으로 추정하고, 거기서 먼 점(배경)을 제거한다.

    **알려진 실패 유형(2026-08-07 실측 확인)**: 평면 임계값을 점군 std로
    자동 추정하는데, 점군에 남아있는 이상치가 std를 부풀리면 임계값이 너무
    커져서 RANSAC이 찾은 "평면"이 전체 점의 절반 가까이를 먹어치우는 사고가
    실제로 발생했다. 그래서 std 대신 MAD(이상치에 강건)로 스케일을 추정하고,
    한 "평면"이 전체 점의 `max_plane_ratio`(기본 40%)를 넘게 먹으면 그 평면은
    버린다. 그래도 마스크가 있으면 항상 마스크 방식을 우선 쓸 것.

**`intersect=True` (실험적 — 기본 비권장, 2026-08-07 위험성 확인)**:
마스크 방식과 기하학적 방식을 둘 다 돌려서 교집합(둘 다 배경이라고 동의한 점)만
제거한다. test02에서는 효과가 좋아 보였지만, test03에서 기하학적 평면 제거가
(발이 바닥에 닿아 있어 발 표면 일부가 바닥과 거의 동일 평면에 놓인 탓에)
**발 자체의 일부를 배경으로 오분류**했고, 교집합(AND)이 마스크의 올바른 판단을
덮어써서 발이 실제로 깎여나가는 게 실측으로 확인됐다. **평면 방식이 마스크
실루엣 내부를 절대 건드리지 않는다는 보장이 없는 한 기본으로 쓰지 말 것**.

**`cluster=True`(DBSCAN, 가장 큰 군집만 남김)**: 마스크 분류(교집합 없이)
뒤에 적용하면 안전하게 잔여 파편을 추가로 제거한다(test02/test03 확인,
발 자체가 쪼개지는 부작용 없음). **주의**: 원본 sparse 포인트클라우드에 바로
걸면 피사체 표면 자체가 국소적으로 듬성듬성해서 군집화가 발을 여러 조각으로
쪼개는 부작용이 있었다(예전 실측) — 반드시 마스크 정리 이후에만 켤 것.

**최종 권장 조합: `clean_point_cloud(..., masks_dir=..., cluster=True)` (교집합 없이).**
"""
...
def classify_by_mask(
    ...
    *,
    ...
    평면 가정이 전혀 없어 기하학적 방식의 실패 유형(평면이 발을 가로질러
    잘라내는 것, 모듈 docstring 참고)이 구조적으로 불가능하다. 특징점 추출이
    아니라 이미 만들어진 재구성 결과의 점을 사후 분류만 하는 것이라 등록률에도
    영향을 주지 않는다.

    Returns:
        `point_ids`와 같은 순서의 (N,) bool 배열 — True면 발로 분류됨.
    """
...
def plane_removal_keep_mask(
    ...
    *,
    ...
    max_plane_ratio: 찾은 "평면"이 전체 점의 이 비율을 넘게 먹으면 진짜
        ...
        가능성이 높다고 보고 버린다(2026-08-07 실측: 이 안전장치 없이
        하나의 "평면"이 전체 점의 49.5%를 먹어치우며 피사체까지 잘라낸
        사고가 실제로 발생함).
...
def estimate_focus_point(recon: pycolmap.Reconstruction) -> np.ndarray:
    """카메라들의 시선(viewing ray)이 가장 가깝게 모이는 공통 지점을 추정한다.

    피사체를 중심으로 돌며 촬영했다면, 각 카메라의 시선은 대략 피사체를 향한다.
    "여러 직선에 가장 가까운 한 점"을 최소제곱으로 구하는 고전적인 문제로 푼다
    (각 광선에 수직인 투영 성분의 제곱합을 최소화).

    "가장 가까운 카메라와의 거리"보다 이게 더 원칙적인 이유: 배경 점도 카메라
    바로 옆(광축에서 살짝 벗어난 곳)에 있으면 카메라와는 가까울 수 있지만,
    카메라들이 실제로 겨냥한 지점과는 멀다 — 이 함수는 후자를 직접 구한다.
    """
...
def clean_point_cloud(
    ...
    *,
    ...
    (모듈 docstring 참고). `intersect=True`는 실험적 옵션으로 발 형상을
    깎아낼 위험이 실측으로 확인됐다 — 기본으로 켜지 말 것.
