...
(`ssm-pipeline-build-notes` 메모리 참고). 대신 이 모듈은 이미 검증된
`deformer.FootMeshDeformer`(실제 발 템플릿을 계측치 몇 개로 매끄럽게 스케일링)를
SfM 점군에 연결한다:

    1. 점군을 템플릿의 좌표계로 강체 정렬(회전+이동)한다 — PCA로 축을 맞추고
       ICP로 다듬는다. 스케일은 점군 자신의 PCA 길이를 250mm 기준으로 임시
       정규화한다(monocular SfM은 절대 축척을 모른다 — 진짜 크기는 자기신고
       사이즈로 추후 보정 필요).
    ...
    3. 그 계측치를 `FootMeshDeformer.deform_from_measurements()`에 넣어 실제
       발 템플릿을 변형한 메쉬를 만든다 — 대응점이 없으니 뭉개질 일도 없다.

**좌우(카이랄리티) 주의**: 발은 카이랄 형상이라 회전만으로는 좌우가 다른
템플릿에 정렬할 수 없다(2026-08-07 test00+S0001 조합에서 실측 확인된 실패
모드 — `lr-chirality-fix-and-heel-bug-isolation` 메모리 참고). 입력 점군과
템플릿의 실제 좌우를 알고 있다면 반드시 `side`/`template_side`를 넘길 것 —
