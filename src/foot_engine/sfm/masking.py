...
SIFT가 좋아하는 특징점이라, `skin_erode`로 마스크를 깎아도 그 경계선을 따라
얇은 고리 모양 잔여 오염이 남을 수 있다(`skin-refine-masking-shipped` 메모리
참고). 근본적으로는 촬영 시 발목 위로 옷이 보이지 않게 하는 것이 가장 확실한
해결책이다.
"""
...
def skin_only_mask(segmenter, bgr_image: np.ndarray, *, erode: int = 8) -> np.ndarray:
    ...
    되는 게 실측으로 확인됐다(2026-08-07, `test00-sleeve-cuff-contamination`
    메모리 참고 — 원인 규명 뒤, 정제를 넣고 나서도 잔여 고리가 남아 재조사).
